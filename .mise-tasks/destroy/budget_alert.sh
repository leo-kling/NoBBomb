...
gcloud billing budgets list --billing-account "$BILLING_ACCOUNT_ID" --billing-project="$GCP_PROJECT_ID" --format="value(name)" --filter="displayName:$BUDGET_ALERT_NAME" | \
  awk -F/ '{print $NF}' | while read -r BUDGET_ID; do
    if [ -n "$BUDGET_ID" ]; then
      gcloud billing budgets delete "$BUDGET_ID" --billing-account "$BILLING_ACCOUNT_ID" --billing-project="$GCP_PROJECT_ID" --quiet
    fi
  done
